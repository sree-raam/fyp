...
======================
OPENCLAW — LLM-ASSISTED AUTONOMOUS INSPECTION BENCHMARK  v16 V7-BASE 9CYL STABLE
...
class CFG:
    ...
    OLLAMA_TIMEOUT  = 25           # seconds per LLM call — fallback will move if Ollama is down
    ...
    CYL_MAX_COUNT = 9      # fixed Gazebo layout has 9 labelled cylinders
    USE_FIXED_CYLINDERS = True  # v16: robot knows labels/coordinates; LiDAR only avoids danger
...
class CylinderRegistry:
    ...
    def load_known_map(self) -> None:
        """Load the fixed 9-cylinder coordinates used by the Gazebo hexagon board.
        These are planning/dashboard targets. LiDAR is still used only for danger distance.
        Override any coordinate from terminal with OPENCLAW_CYL_1_X, OPENCLAW_CYL_1_Y, etc.
        """
        defaults = [
            ("CYL_1",  0.00,  1.75, 0.12),  # top centre
            ("CYL_2",  0.00,  0.85, 0.12),  # upper centre
            ("CYL_3",  0.00,  0.00, 0.12),  # centre
            ("CYL_4", -0.80, -0.85, 0.12),  # lower left
            ("CYL_5",  0.00, -0.85, 0.12),  # lower centre
            ("CYL_6",  0.80, -0.85, 0.12),  # lower right
            ("CYL_7",  0.80,  0.00, 0.12),  # centre right
            ("CYL_8",  0.80,  0.85, 0.12),  # upper right
            ("CYL_9", -0.80,  0.00, 0.12),  # centre left
        ]
        now = time.time()
        with self._lock:
            self._cyls.clear()
...
def _explore_for_cylinders(budget: float) -> None:
    """Explorer button for fixed 9-cylinder board.
    It only reloads/shows CYL_1..CYL_9. It does NOT drive the robot around.
    This prevents aborted/stuck EXPLORING while keeping the v7 movement core.
    """
    ST.set_state("MAP_READY")
    _REGISTRY.load_known_map()
    ST.log("EXPLORE: fixed 9-cylinder Gazebo map loaded. LiDAR remains safety-only.")
    time.sleep(0.2)
    ST.set_state("IDLE")
...
def _fallback_actions_from_text(mission_text: str) -> List[Dict[str, Any]]:
    """Deterministic fallback so the robot still moves even if Ollama is slow/offline.
    Supports CYL_1..CYL_9, "cylinder 4", nearest/far/all, turn, forward, home.
    """
    t = mission_text.lower()
    actions: List[Dict[str, Any]] = []

    # Specific cylinder numbers first: "inspect cylinder 4", "go to CYL_7", etc.
    nums: List[int] = []
    for m in re.finditer(r'(?:cyl[_\s-]*|cylinder\s*)([1-9])', t):
        n = int(m.group(1))
        if n not in nums:
            nums.append(n)
    if nums and ("inspect" in t or "go" in t or "visit" in t or "move" in t or "cylinder" in t):
        for n in nums:
    ...
    # Movement primitives from v7 remain unchanged.
    if "turn left" in t or ("left" in t and "inspect" not in t and "cylinder" not in t):
        actions.append({"type": "turn_left", "degrees": 90})
    if "turn right" in t or ("right" in t and "inspect" not in t and "cylinder" not in t):
        actions.append({"type": "turn_right", "degrees": 90})
    if "straight" in t or "forward" in t or "go straight" in t or ("move" in t and not nums):
...
def plan_actions(model: str, mission_text: str) -> Dict[str, Any]:
    ...
    for a in actions[:12]:
        ...
        if typ == "move_forward":
            ...
            cid = str(a.get("id", "")).upper().replace(" ", "_")
            m = re.search(r'([1-9])', cid)
            if m:
                clean.append({"type": typ, "id": f"CYL_{int(m.group(1))}", "seconds": max(1.0, min(5.0, float(a.get("seconds", 2))))})
...
def _select_cylinders_for_action(action_type: str, cyl_id: str = "") -> List[TrackedCylinder]:
    cyls = _REGISTRY.confirmed_list()
    if not cyls:
        return []
    if action_type == "inspect_cylinder" and cyl_id:
        c = _REGISTRY.get_by_name(cyl_id)
        return [c] if c else []
    with ST.lock:
        rx, ry = ST.rx, ST.ry
...
def run_single_mission(model: str, mission_text: str,
    ...
    for action in actions:
        ...
        if typ == "move_forward":
            ...
            targets = _select_cylinders_for_action(typ, str(action.get("id", "")))
            if not targets:
...
_TOPBAR = """<div class="topbar">
  <div class="logo">&#129302; OPENCLAW <span class="tb-badge">Benchmark v16</span></div>
...
def _watchdog() -> None:
    while True:
        ...
        if n_conf == 0 and mstate in ("IDLE", "EXPLORING"):
            ST.log("WATCHDOG: fixed cylinder map unavailable — press Explore/Reload Map")
...
def main() -> None:
    global NODE
    print("=" * 66, flush=True)
    print("OPENCLAW — LLM-Assisted Autonomous Inspection Benchmark v16", flush=True)
    ...
    print(f"Cyls   : fixed Gazebo map CYL_1..CYL_{CFG.CYL_MAX_COUNT}; LiDAR safety only", flush=True)
