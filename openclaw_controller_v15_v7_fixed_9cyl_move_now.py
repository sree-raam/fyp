...
======================
OPENCLAW — LLM-ASSISTED AUTONOMOUS INSPECTION BENCHMARK  v15 (v7 working movement + fixed 9-cylinder map)
...
Architecture (NEW — not patched from old code):
  ...
  ├─ CylinderRegistry   Fixed 9-cylinder map + stable CYL_N labels
...
Key design decisions vs old code:
  ...
  • Cylinders: fixed 9-cylinder Gazebo map; LiDAR used for danger/obstacle avoidance only
...
class CFG:
    ...
    CYL_MAX_COUNT = 9      # current Gazebo layout has 9 cylinders
    USE_FIXED_CYLINDERS = True  # robot already knows all 9 cylinder locations; LiDAR is safety only
...
class CylinderRegistry:
    ...
    def reset(self) -> None:
        with self._lock:
            self._cyls.clear()
            self._next = 1
            if getattr(CFG, "USE_FIXED_CYLINDERS", False):
                self.load_known_map()

    def load_known_map(self) -> None:
        """Load the fixed 9-cylinder Gazebo layout.
        These are dashboard/planning waypoints, not extra physical obstacles.
        Tune using OPENCLAW_CYL*_X/Y if your Gazebo coordinates differ.
        """
        now = time.time()
        defaults = [
        ...
        self._cyls.clear()
        self._next = 1
        for idx, (label, x, y, radius) in enumerate(defaults, 1):
            x = float(os.environ.get(f"OPENCLAW_{label}_X", str(x)))
            y = float(os.environ.get(f"OPENCLAW_{label}_Y", str(y)))
            self._cyls[idx] = TrackedCylinder(
                cyl_id=idx, x=x, y=y, radius=radius,
                observe_count=99, confirmed=True, last_seen=now, label=label
            )
            self._next = idx + 1
...
class OpenClawNode(Node):
    ...
    def _scan_cb(self, msg: LaserScan) -> None:
        ...
        # LiDAR is used for obstacle danger only in the fixed 9-cylinder map mode.
        # Do not let noisy scans rename or move known cylinder coordinates.
        if not getattr(CFG, "USE_FIXED_CYLINDERS", False):
...
def _explore_for_cylinders(budget: float) -> None:
    """Explorer button for this fixed Gazebo world.
    It does NOT drive the robot around anymore. The robot already knows CYL_1..CYL_9.
    This prevents the old stuck/aborted EXPLORING state.
    """
    ST.set_state("MAP_READY")
    if getattr(CFG, "USE_FIXED_CYLINDERS", False):
        _REGISTRY.load_known_map()
        ST.log("EXPLORE: fixed 9-cylinder map loaded. LiDAR remains active for safety only.")
    else:
        ST.log("EXPLORE: dynamic discovery disabled in this stable data-collection build.", "WARN")
    time.sleep(0.2)
    ST.set_state("IDLE")
...
def _fallback_actions_from_text(mission_text: str) -> List[Dict[str, Any]]:
    """Deterministic fallback so the robot still moves even if Ollama fails.
    Supports CYL_1..CYL_9, "cylinder 4", nearest/far/all, turn, forward, home.
    """
    t = mission_text.lower()
    actions: List[Dict[str, Any]] = []

    # Specific cylinder numbers have priority, e.g. "inspect cylinder 4" or "CYL_4".
    nums = []
    for m in re.finditer(r'(?:cyl[_\s-]*|cylinder\s*)([1-9])', t):
        n = int(m.group(1))
        if n not in nums:
            nums.append(n)
    if nums and ("inspect" in t or "go" in t or "visit" in t):
        for n in nums:
            actions.append({"type": "inspect_cylinder", "id": f"CYL_{n}", "seconds": 2})

    if not actions and ("inspect" in t or "cylinder" in t):
        if "far" in t:
            actions.append({"type": "inspect_far", "seconds": 2})
        elif "all" in t or "every" in t or "each" in t:
            actions.append({"type": "inspect_all", "seconds": 2})
        else:
            actions.append({"type": "inspect_nearest", "seconds": 2})

    if "turn left" in t or ("left" in t and "inspect" not in t):
        actions.append({"type": "turn_left", "degrees": 90})
    if "turn right" in t or ("right" in t and "inspect" not in t):
        actions.append({"type": "turn_right", "degrees": 90})
    if "straight" in t or "forward" in t or "go straight" in t or "move" in t:
    ...
    if "home" in t or "return" in t or "reset" in t or "ideal" in t:
...
def plan_actions(model: str, mission_text: str) -> Dict[str, Any]:
    ...
    allowed = (
        ...
        'turn_right {"degrees":15..180}, inspect_cylinder {"id":"CYL_1".."CYL_9","seconds":2}, '
        'inspect_nearest {"seconds":2}, inspect_far {"seconds":2}, inspect_all {"seconds":2}, return_home'
    )
    prompt = (
    ...
    for a in actions[:12]:
        ...
        if typ == "move_forward":
            ...
            cid = str(a.get("id", a.get("label", ""))).strip().upper().replace(" ", "_")
            m = re.search(r"([1-9])", cid)
            cid = f"CYL_{m.group(1)}" if m else ""
            if cid in [f"CYL_{i}" for i in range(1, 10)]:
                clean.append({"type": typ, "id": cid, "seconds": max(1.0, min(5.0, float(a.get("seconds", 2))))})
    ...
    with ST.lock:
        ST.planned_route = [(a["type"] + (":" + a.get("id", "") if a.get("id") else "")) for a in clean]
        ST.reasoning = reasoning
    return {"actions": clean, "reasoning": reasoning, "raw": raw, "llm_latency": latency, "parse_time": 0.0, "valid": True}


def _select_cylinders_for_action(action_type: str, action: Optional[Dict[str, Any]] = None) -> List[TrackedCylinder]:
    cyls = _REGISTRY.confirmed_list()
    if not cyls:
        return []
    if action_type == "inspect_cylinder" and action:
        wanted = str(action.get("id", "")).upper().replace(" ", "_")
        c = _REGISTRY.get_by_name(wanted)
        return [c] if c else []
    with ST.lock:
        rx, ry = ST.rx, ST.ry
...
def run_single_mission(model: str, mission_text: str,
    ...
    for action in actions:
        ...
        if typ == "move_forward":
            ...
            with ST.lock: target = ST.ryaw - math.radians(float(action.get("degrees", 90)))
            ok = (NODE.align_to_heading(target, deadline) == "ARRIVED")
        elif typ in ("inspect_nearest", "inspect_far", "inspect_all", "inspect_cylinder"):
            targets = _select_cylinders_for_action(typ, action)
            if not targets:
...
def run_benchmark(models: List[str], runs: int, mission: str,
    ...
    if not available:
        ST.log("BENCHMARK WARNING: Ollama models not warmed. Runs will still start and invalid LLM calls will be logged/fallbacked.", "WARN")
...
function drawArena(cv,d,showRobot,heatPts){
  ...
  cyls.forEach(c=>{
    ...
    ctx.fillStyle='#0f172a';ctx.font='bold 7px monospace';ctx.textAlign='center';
    ctx.fillText(c.label.replace('CYL_','C'),p[0],p[1]+2.5);ctx.textAlign='left';
  });
  // HOME
  const hp=T.w2c(HOME_PT.x,HOME_PT.y);
...
def api_mission():
    ...
    return jsonify({"status": "started", "model": model, "mission": mission})
...
def api_reset_cylinders():
    _REGISTRY.reset()
    ST.log("Cylinder map reset/reloaded: CYL_1..CYL_9 fixed Gazebo layout ready.")
    return jsonify({"status": "fixed_9cylinder_map_reloaded"})
