...
class Config:
    ...
    OLLAMA_TIMEOUT_S = float(os.environ.get("OPENCLAW_OLLAMA_TIMEOUT", "180"))
    ...
    RUNS_PER_MODEL = int(os.environ.get("OPENCLAW_RUNS_PER_MODEL", "20"))
    ...
    MAX_LINEAR_SPEED = float(os.environ.get("OPENCLAW_MAX_LINEAR", "0.20"))
    MAX_ANGULAR_SPEED = float(os.environ.get("OPENCLAW_MAX_ANGULAR", "0.75"))
    ...
    ARRIVAL_RADIUS = float(os.environ.get("OPENCLAW_ARRIVAL_RADIUS", "0.25"))
    HOME_RADIUS = float(os.environ.get("OPENCLAW_HOME_RADIUS", "0.28"))
    ...
    OBSTACLE_STOP_M = float(os.environ.get("OPENCLAW_OBSTACLE_STOP", "0.24"))
    OBSTACLE_SLOW_M = float(os.environ.get("OPENCLAW_OBSTACLE_SLOW", "0.55"))
    ...
    CYL_CONFIRM_HITS = int(os.environ.get("OPENCLAW_CYL_CONFIRM_HITS", "3"))
...
class SmoothNavigator:
    ...
    def drive_to(self, goal: Tuple[float, float], arrival_radius: float, timeout_s: float, target_label: str = "") -> bool:
        ...
        while time.time() - start < timeout_s:
            ...
            angular = clamp(1.25 * heading_error, -Config.MAX_ANGULAR_SPEED, Config.MAX_ANGULAR_SPEED)

            if front < Config.OBSTACLE_STOP_M:
                linear = 0.0
                angular = self._avoidance_turn(left, right)
                self.replan_count += 1
            elif front < Config.OBSTACLE_CLEAR_M:
                linear *= 0.35
                angular += 0.35 * self._avoidance_turn(left, right)
    ...
    def perception_sweep(self, duration_s: float = 12.0) -> None:
        ...
        while time.time() - start < duration_s:
            if STATE.stop_requested or CYLINDERS.ready():
                break
            _, angular = self._limit_accel(0.0, 0.32, dt)
    ...
    def return_home(self) -> bool:
        ...
        ok = self.drive_to((Config.HOME_X, Config.HOME_Y), Config.HOME_RADIUS, Config.WAYPOINT_TIMEOUT_S, "HOME")
        if ok:
            self.align_heading(Config.HOME_YAW, timeout_s=8.0)
        self.node.stop_robot()
        verified = ok and distance(self.node.get_pose().xy(), (Config.HOME_X, Config.HOME_Y)) <= Config.HOME_RADIUS
        if verified:
            with STATE.lock:
    ...
    def align_heading(self, yaw: float, timeout_s: float) -> bool:
        ...
        while time.time() - start < timeout_s:
            ...
            if abs(err) < 0.07:
                self.node.stop_robot()
                return True
            _, angular = self._limit_accel(0.0, clamp(1.1 * err, -0.45, 0.45), dt)
            self.node.publish_cmd(0.0, angular)
            time.sleep(dt)
        self.node.stop_robot()
        return False
    ...
    def _linear_profile(self, dist: float, heading_error: float, front: float) -> float:
        turn_factor = max(0.0, 1.0 - abs(heading_error) / 1.1)
        stop_factor = clamp((dist - Config.ARRIVAL_RADIUS) / 0.9, 0.0, 1.0)
        obstacle_factor = clamp((front - Config.OBSTACLE_STOP_M) / (Config.OBSTACLE_SLOW_M - Config.OBSTACLE_STOP_M), 0.0, 1.0)
        return Config.MAX_LINEAR_SPEED * turn_factor * stop_factor * obstacle_factor

    def _avoidance_turn(self, left: float, right: float) -> float:
        return 0.55 if left >= right else -0.55
...
{{
  ...
  "route": ["CYL_1", "CYL_2", "CYL_3", "CYL_4", "CYL_5", "CYL_6"]
}}
The route must include every listed cylinder exactly once.
"""
    ...
    def _parse(self, text: str, labels: List[str]) -> Tuple[List[str], str]:
        ...
        if not route:
            route = [x.upper() for x in re.findall(r"CYL_[1-6]", text.upper())]
        normalized = []
        for label in route:
...
class MissionEngine:
    ...
    def _ensure_home_before_run(self) -> bool:
        ...
        if not self._wait_for_ros_inputs():
            STATE.log("Waiting for ROS /odom and /scan failed before run start.", "warning")
            return False
    ...
    def run_single(self, model: str, command: str) -> RunMetrics:
        ...
        try:
            ...
            if not self._discover_cylinders():
                raise RuntimeError("failed to discover all six cylinders")
            ...
            if not ok:
                raise RuntimeError("route execution cancelled or failed")

            result.return_home_success = NAVIGATOR.return_home()
            if not result.return_home_success:
                raise RuntimeError("return-home verification failed")
            result.success = True
            with STATE.lock:
            ...
            STATE.log(f"Run {result.run_number} for {model} complete: success={result.success}", "info")
        return result

    def _discover_cylinders(self) -> bool:
        start = time.time()
        STATE.log("Starting LiDAR cylinder discovery.", "info")
        if CYLINDERS.ready():
            STATE.log("Six cylinders already discovered.", "info")
            return True
        patrol = [
            (Config.HOME_X + 0.55, Config.HOME_Y),
            (Config.HOME_X + 0.55, Config.HOME_Y + 0.55),
            (Config.HOME_X, Config.HOME_Y + 0.55),
            (Config.HOME_X - 0.55, Config.HOME_Y + 0.55),
            (Config.HOME_X - 0.55, Config.HOME_Y),
            (Config.HOME_X, Config.HOME_Y),
        ]
        idx = 0
        while time.time() - start < Config.DISCOVERY_TIMEOUT_S:
            if STATE.stop_requested:
                return False
            if CYLINDERS.ready():
                STATE.log("Cylinder discovery complete.", "info")
                return True
            NAVIGATOR.perception_sweep(duration_s=8.0)
            if CYLINDERS.ready():
                break
            target = patrol[idx % len(patrol)]
            idx += 1
            NAVIGATOR.drive_to(target, 0.22, 18.0, "DISCOVERY")
            time.sleep(0.2)
        ready = CYLINDERS.ready()
        if not ready:
            STATE.log(f"Discovery incomplete: {len(CYLINDERS.discovered())}/{Config.EXPECTED_CYLINDERS} cylinders confirmed.", "warning")
        return ready
...
def create_app() -> Flask:
    ...
    def api_start_benchmark() -> Response:
        ...
        runs = int(payload.get("runs") or Config.RUNS_PER_MODEL)
...
<body>
  <header>
    ...
    <nav>
      <a href="/">Mission</a><a href="/individual">Individual</a><a href="/comparison">Comparison</a><a href="/export">Export</a>
    </nav>
  </header>
  <main>
    <div class="stack">
      <section>
        ...
        <div class="row" style="margin-top:8px">
          ...
          <button onclick="startBenchmark()">20 x 3</button>
...
function startBenchmark(){ post("/api/start_benchmark",{models, runs:20, command:document.getElementById("command").value}); }
...
function drawMap() {
  ...
  const home=state.home||[0,0]; ctx.fillStyle="#66c27a"; ctx.beginPath(); ctx.arc(tx(home[0],w),ty(home[1],h),9,0,Math.PI*2); ctx.fill(); ctx.fillText("HOME",tx(home[0],w)+12,ty(home[1],h));
  (state.cylinders||[]).forEach(cy=>{ ctx.fillStyle=cy.discovered?"#6aa9ff":"#607080"; ctx.beginPath(); ctx.arc(tx(cy.x,w),ty(cy.y,h),10,0,Math.PI*2); ctx.fill(); ctx.fillStyle="#eef3f8"; ctx.fillText(cy.label,tx(cy.x,w)+13,ty(cy.y,h)+4); });
