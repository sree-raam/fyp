    ...
    def reset_robot(self) -> bool:
        ...
        with CONTROL_LOCK:
            ...
            for _ in range(6):
                ...
                time.sleep(0.04)
            time.sleep(0.25)
            # If already inside HOME tolerance, do not fight the robot. Just align softly and finish.
            with ST.lock:
                start_d = math.hypot(ST.rx - CFG.HOME_X, ST.ry - CFG.HOME_Y)
            if start_d <= CFG.HOME_TOL:
                ST.log(f"RESET: already at HOME zone dist={start_d:.2f}m — no drive needed")
